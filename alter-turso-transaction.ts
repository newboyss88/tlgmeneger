...
async function addColumn() {
  try {
    await remoteDb.execute('ALTER TABLE "Transaction" ADD COLUMN "telegramUserId" TEXT');
    console.log("✅ Transaction jadvaliga 'telegramUserId' ustuni qo'shildi!");
  } catch (e: any) {
    if (e.message.includes('duplicate column')) {
      console.log("ℹ️ 'telegramUserId' ustuni allaqachon mavjud.");
    } else {
